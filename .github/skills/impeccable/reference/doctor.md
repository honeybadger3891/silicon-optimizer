...
- **Tool version.** The installed skill is older than the published one. `context.mjs` reports that at boot as `UPDATE_AVAILABLE`; update it through the installed plugin or skill manager in a new session after the user explicitly asks. Not this command's job.
