...
Manual `node <skill-base-dir>/scripts/detect.mjs` scans use the same project filter config by default: `detector.ignoreRules`, `detector.ignoreFiles`, `detector.ignoreValues`, and `detector.designSystem.enabled`. `hook.enabled` only controls automatic hook execution, not manual CLI scans. Add `--no-config` for a raw detector run that ignores project config/context. Use the bundled `hook-admin.mjs` actions below for detector-ignore CRUD.
...
- If the finding has no value-specific command, such as `side-tab`, scope that one rule to the file: `ignore-value <id> "*" --file <path>`. Run `node <skill-base-dir>/scripts/detect.mjs <path>` first to see what actually fires there.
